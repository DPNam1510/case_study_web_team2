...
public class AdminMedicalFormsRepo implements IAdminMedicalFormsRepo{
    private final String GET_LIST_PAY_NOT_YET = "SELECT mf.id AS forms_id, c.name AS customer_name, " +
            "NULL AS service_name, NULL AS doctor_name, mf.status " +
            "FROM medical_forms mf " +
            "JOIN customer c ON mf.customer_id = c.id " +
            "left JOIN forms_detail fd ON mf.id = fd.forms_id " +
            "WHERE mf.status = 'Completed' and " +
    ...
    private final String LIST_APPROVE = "select mf.id as forms_id,c.name as customer_name,s.name as service_name,s.doctor_name ,mf.status as status\n" +
            "from medical_forms mf\n" +
            "join customer c on mf.customer_id = c.id\n" +
    ...
    public List<AdminMedicalFormsDto> getListPayNotYet() {
        ...
        try(Connection connection = ConnectDB.getConnectDB()) {
            PreparedStatement preparedStatement = connection.prepareStatement(GET_LIST_PAY_NOT_YET);
            ResultSet resultSet = preparedStatement.executeQuery();
            while (resultSet.next()){
                int id = resultSet.getInt("forms_id");
                String customerName = resultSet.getString("customer_name");
